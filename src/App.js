...
class App extends Component {
  constructor(props) {
    ...
    this.filterStopWatching = this.filterStopWatching.bind(this);
    this.contractEventWatchStop = this.contractEventWatchStop.bind(this);
  ...
  componentWillMount() {
    ...
    if (typeof web3 !== 'undefined') {
      ...
      // var provider = cument.getElementById('provider_url');
  ...
  startApp() {
    ...
    this.filterStopWatching();
    this.contractEventWatchStop();
 ...
 * The Provider URL is provided in a cument element with the 
 * id = provider_url
 */
   connect(providerURL) {
  ...
  filterStopWatching() {
    console.log("called filterStopWatching");
  }

  contractEventWatchStop() {
    console.log("called contractEventWatchStop");
  }

  setData(cElement, html, errored) {
    console.log(cElement, html, "error:" + errored);  
  }

  updateStateFromChild(data) {
    console.log('called updateStateFromChild');
    this.setState({ savedContract: data });
  }
  ...
  render() {
    return (
      <main>
        <About />
        <SetupAndVersion state={this.state} reConnect={this.connect}  />
        <Accounts />
        <LockUnlock />
        <SendTransaction />
        <CompileDeployContract abi={this.state.contract_abidefinition} byteCode = {this.state.contract_bytecode} update={this.updateStateFromChild}/>
        <ExecuteContractFunction abi={this.state.contract_abidefinition} byteCode = {this.state.contract_bytecode} savedContract={this.state.savedContract}/>
        <FilterOptionsWatch />
        <Events />
    </main>
    );
